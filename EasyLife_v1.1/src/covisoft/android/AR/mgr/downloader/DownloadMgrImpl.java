 ...
 */
package covisoft.android.AR.mgr.downloader;
...
import covisoft.android.AR.data.convert.DataConvertor;
import covisoft.android.AR.mgr.HttpTools;
...
class DownloadMgrImpl implements Runnable, DownloadManager {
	...
	private DownloadResult processRequest(ManagedDownloadRequest mRequest) {
		...
		try {
			...
			String pageContent = HttpTools.getPageContent(request, ctx.getContentResolver());

			if (pageContent != null) {
				// try loading Marker data
				request.getSource().getUrl();
				List<Marker> markers = DataConvertor.getInstance().load(request.getSource().getUrl(), pageContent, request.getSource());
