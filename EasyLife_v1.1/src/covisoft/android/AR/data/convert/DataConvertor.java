...
public class DataConvertor {
	...
	public List<Marker> load(String url, String rawResult, DataSource ds){
		...
		if (MixView.type.equals("All")) {
			if (url.contains("easylife")) {
				dataProcessor = new EzLifeDataProcessor();
			} else if(dataProcessor == null){
				dataProcessor = new MixareDataProcessor(); //using this as default if nothing is found.
			}
		} else if (MixView.type.equals("Single")){
			if (url.contains("easylife")) {
				dataProcessor = new EzLifeDataProcessor_Single();
			} else if(dataProcessor == null){
				dataProcessor = new MixareDataProcessor(); //using this as default if nothing is found.
			}
		}
...
//		dataProcessors.add(new OsmDataProcessor());
		dataProcessors.add(new EzLifeDataProcessor());
	}
