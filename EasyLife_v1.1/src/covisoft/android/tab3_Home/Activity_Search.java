...
/*
 * Activity search shop in Small Category
 ...
 *           - Use EZUtil Progress
 */
public class Activity_Search extends NavigationActivity {
	...
	private ArrayList<item_store_list_search> arItem = new ArrayList<item_store_list_search>();
	// List shop show in list
	private ArrayList<item_store_list_search> arValue = new ArrayList<item_store_list_search>();
...
//  ==========================================================================================================
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.layout_08_search);
		...
		editSearch.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				...
				if (editSearch.getText().toString().length() > 0
						&& btnClear.getVisibility() == View.GONE) {
				...
				if(!EZUtil.isLoading) {
					...
					if (keyword.equals("")) {
						if (btnSearchName.isChecked()) {
							...
							showPopupOneOption(Activity_Search.this.getResources().getString(R.string.popup_CannotSearchName), 1);
							
						} else if (btnSearchLocation.isChecked()) {
							
							showPopupOneOption(Activity_Search.this.getResources().getString(R.string.popup_CannotSearchLocation), 1);
							
						} else if (btnSearchProduct.isChecked()) {
							
							showPopupOneOption(Activity_Search.this.getResources().getString(R.string.popup_CannotSearchProduct), 1);
	...
	private class AsyncTaskSearch extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			if(type.equals("Name")) {
				service_Search xml = new service_Search(activity, keyword, "name", category);
				arItem = xml.start();
			} else if(type.equals("Location")) {
				service_Search xml = new service_Search(activity, keyword, "location", category);
				arItem = xml.start();
			} else if(type.equals("Product")) {
				service_Search xml = new service_Search(activity, keyword, "product", category);
				arItem = xml.start();
			}
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							  public void run() {
								  showPopupOneOption(activity.getResources().getString(R.string.popup_Search_EmptyList), 1);
							  }
						});
...
//=============================================================================================	
	private void init_listview_Name() {
		...
		if(listResult_Name.getAdapter() != null) {
			...
			adapter_Name = new Adapter_Search(Activity_Search.this, arValue, 2);
	...
	private void init_listview_Location() {
		...
		if(adapter_Location != null) {
			...
			adapter_Location = new Adapter_Search(Activity_Search.this, arValue, 2);
	...
	private void init_listview_product() {
		...
		if(adapter_Product != null) {
			...
			adapter_Product = new Adapter_Search(Activity_Search.this, arValue, 2);
