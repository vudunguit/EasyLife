...
public class Activity_Login extends NavigationActivity {
...
// ===================================================================================================
	...
	public void initBtnBack() {
		...
		layoutBack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Log.e("Onbackpress", "Login: btnBack");
				onBackPressed();
			}
		});
	}
	...
	/* 
	 * Check where to go when back
	 *     'settingGo', 'Home', 'itemDetail'  : Back
	 *     'Activity_Coupon'                  : Check if loged, hide ln_Check comment -> Back
	 *   
	 */
	@Override
	...
	private class AsyncTaskRequestData extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			service_Login xml = new service_Login(Activity_Login.this, username, password);
			loginResult = xml.start();
			...
			if (loginResult.equals("2")) {
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login.this.getResources().getString(R.string.popup_LoginFail_cannotConnect), 1);
							}
						});
					}
				}, 300);
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login.this.getResources().getString(R.string.popup_LoginFail), 1);
							}
						});
					}
				}, 300);
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login.this.getResources().getString(R.string.popup_cannot_conect), 1);
							}
						});
					}
				}, 300);
	...
	public void initBtnForgotPass() {
		...
		btnForgot.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent(Activity_Login.this, Activity_ForgotPass_Tab5.class);
				intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
	...
	/*
	 * Init & Implement Button Facebook Login
	 */
			...
			btnFBLogin.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (!Utility.mFacebook.isSessionValid()) {
						Toast.makeText(Activity_Login.this, "Authorizing", Toast.LENGTH_SHORT).show();
						Utility.mFacebook.authorize(Activity_Login.this.getParent(), permission, new LoginDialogListener());
					} else {
						activity.runOnUiThread(new Runnable() {
							public void run() {
								if (getParent() != null) {
						...
						try {
							...
							SessionStore.save(Utility.mFacebook, Activity_Login.this);
	...
	public final class LoginDialogListener implements DialogListener {
		...
		public void onFacebookError(FacebookError error) {
			btnFBLogin.setVisibility(View.VISIBLE);
			Toast.makeText(Activity_Login.this, "Something went wrong. Please try again.", Toast.LENGTH_LONG).show();
		}
    ...
    private class AsyncTask_LoginFB extends AsyncTask<Void, Void, item_FBLogin> {
		...
		protected item_FBLogin doInBackground(Void... params) {
			...
			service_Login_FB xml = new service_Login_FB(Activity_Login.this, fbEmail, fbUsername, fbFname, fbGender, fbImg);
			...
			if (result.getResult().equals("1")) {
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login.this.getResources().getString(R.string.popup_LoginFB_ProblemConnect), 1);
							}
						});
					}
				}, 300);
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login.this.getResources().getString(R.string.popup_LoginFB_New), 2);
							}
						});
					}
				}, 300);
