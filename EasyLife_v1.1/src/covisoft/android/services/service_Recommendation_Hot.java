...
public class service_Recommendation_Hot {
	...
	public service_Recommendation_Hot(Activity act) {
		this.act = act;
		sb = new StringBuffer();
		sb.append(act.getResources().getString(R.string.service_recommandation_list_Vip));
		Log.e("Get recommandation list VIP", sb.toString());
	}
			...
			while (parserEvent != XmlPullParser.END_DOCUMENT) {
				...
				case XmlPullParser.END_TAG:
					...
					if (tag_end.compareTo("item") == 0) {
						arItem.add(new item_Recommandation(no, coupon,
								categoryNo, categoryNoBasic, categoryName,
								name, addr, tel, s_logo, Double.parseDouble(lat), Double.parseDouble(lng), Integer.parseInt(level)));
					}
					break;
