...
/*
 * Activity Login in tab 2
 *             
 * last Updated: 1.07.2013
 *      Updater: Huan
 *      Update Info:
 *             - Optimize Facebook Login
 *             - Use EZUtil Progress
 */

public class Activity_Login_Tab2 extends NavigationActivity {
...
// ===================================================================================================
	...
	public void onBackPressed() {
		...
		if (EasyLifeActivity.user != null ) {
			Intent intent = new Intent(Activity_Login_Tab2.this, Activity_ListCoupon.class);
			intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
			goNextHistory_2("Activity_ListCoupon", intent);
		} else {
	...
	public void init_EditText() {
		...
		if (EasyLifeActivity.user == null && !edtEmail.getText().toString().equals("") && !edtPassword.getText().toString().equals("")) {
		...
		btnLogin.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (edtEmail.getText().toString().trim().equals("")) {
					showPopupOneOption(activity.getString(R.string.popup_LoginFail_FillID), 1);
				} else if (edtPassword.getText().toString().trim().equals("")) {
					showPopupOneOption(activity.getString(R.string.popup_LoginFail_FillPass), 1);
				} else {
					if (EZUtil.isNetworkConnected(activity)) {
						...
						showPopupOneOption(activity.getString(R.string.No_Internet_now), 1);
					}
				}
	...
	private class AsyncTask_Login extends AsyncTask<Void, Void, String> {
		...
		protected String doInBackground(Void... params) {
			...
			service_Login xml = new service_Login(Activity_Login_Tab2.this, username, password);
			...
			if (result.equals("2")) {
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login_Tab2.this.getResources().getString(R.string.popup_LoginFail_cannotConnect), 1);
							}
						});
					}
				}, 300);
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login_Tab2.this.getResources().getString(R.string.popup_LoginFail), 1);
							}
						});
					}
				}, 300);
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login_Tab2.this.getResources().getString(R.string.popup_cannot_conect), 1);
							}
						});
					}
				}, 300);
	...
	/*
	 * 
	 *	Init & Implement Button Register 
	 *
	 */
	void initBtnRegister() {
		...
		btnRegister.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent(Activity_Login_Tab2.this, Activity_Register.class);
				intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
//				intent.putExtra("Login_Type", "Tab2");
	...
	/*
	 * 
	 *	Init & Implement Button Forgot Password 
	 *
	 */
	public void initBtnForgotPass() {
		...
		btnForgot.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent intent = new Intent(Activity_Login_Tab2.this, Activity_ForgotPass_Tab5.class);
				intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
			...
			btnFBLogin.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					if (!Utility.mFacebook.isSessionValid()) {
						Toast.makeText(Activity_Login_Tab2.this, "Authorizing", Toast.LENGTH_SHORT).show();
						Utility.mFacebook.authorize(Activity_Login_Tab2.this.getParent(), permission, new LoginDialogListener());
					} else {
						activity.runOnUiThread(new Runnable() {
							public void run() {
								if (getParent() != null) {
						...
						try {
							...
							SessionStore.save(Utility.mFacebook, Activity_Login_Tab2.this);
	...
	public final class LoginDialogListener implements DialogListener {
		...
		public void onFacebookError(FacebookError error) {
			btnFBLogin.setVisibility(View.VISIBLE);
			Toast.makeText(Activity_Login_Tab2.this, "Something went wrong. Please try again.", Toast.LENGTH_LONG).show();
		}
	...
	public class UserRequestListener extends BaseRequestListener {
		...
		public void onComplete(final String response, final Object state) {
			...
			try {
				...
				Log.e("getUserInfo", "email: " + fbEmail + " - username: " + fbUsername + " - name: " + fbFname + " - gender: " + fbGender + " - Picture: " + fbImg);
	...
	private class AsyncTask_LoginFB extends AsyncTask<Void, Void, item_FBLogin> {
		...
		protected item_FBLogin doInBackground(Void... params) {
			...
			service_Login_FB xml = new service_Login_FB(Activity_Login_Tab2.this, fbEmail, fbUsername, fbFname, fbGender, fbImg);
			...
			if (result.getResult().equals("1")) {
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login_Tab2.this.getResources().getString(R.string.popup_LoginFB_ProblemConnect), 1);
							}
						});
					}
				}, 300);
				...
				t.schedule(new TimerTask() {
					...
					public void run() {
						...
						activity.runOnUiThread(new Runnable() {
							public void run() {
								showPopupOneOption(Activity_Login_Tab2.this.getResources().getString(R.string.popup_LoginFB_New), 2);
							}
						});
					}
				}, 300);
